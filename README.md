...
**An app (L4)** is a small contract: *a backend that owns its own working-set data and reads the brain read-only over HTTP, **+** a PWA built from the web-toolkit, **+** one vhost behind the shared edge.* What makes every app feel like one product is the shared edge, the shared toolkit, and the shared brain — one sign-in, one design system, one source of truth. Because that cohesion lives in the shared layers rather than a shared runtime, each backend is free to fit its job: Python for the brain, Go for scout.

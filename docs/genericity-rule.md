...
- Generic tools (`search_brain`, `get_entity`, `add_episode`) that the agent composes — not workflows baked into the architecture.
- Multiple capture paths (PWA primary, iOS Shortcut as convenience, MCP for terminal agents) — no single "primary user."
...
- A single persistent store ([storage-policy.md](./storage-policy.md)) so a downstream user inherits the minimum possible ops surface.
